					...
					<div class="col-xs-12">
						<div class="row-fluid" style="margin-bottom: 5px;">
							<div class="span12 control-group">
								...
								<my:button className="btn" id="bnt-grant" textName="分配角色" permission="/sys/user" />
							</div>
						</div>
						<!-- PAGE CONTENT BEGINS -->
	...
	<script type="text/javascript">
	        ...
	        $("#bnt-grant").click(function() {
		        ...
		        if(rid == -1) {
			        ...
			        parent.layer.open({
			            ...
			            fix : false, //不固定
			            maxmin : true,
			            content : '${basePath}/sys/user/userRoleSetting?uid=' + rid });
		        }
	        });
	        ...
	        $("#btn-edit").click(function() {//添加页面
		        ...
		        if(rid == -1) {
			        ...
			        parent.layer.open({
			            ...
			            fix : false, //不固定
			            maxmin : true,
			            content : '${basePath}/sys/user/add?id=' + rid });
		        }
	        });
        ...
        function setVisible(status) {
	        var submitData = {
	            "ids" : getSelectedRows(),
	            "visible" : status };
	        $.post("${basePath}/sys/user/setVisible", submitData, function(data) {
		        if(data.code == 0) {
			        layer.msg("操作成功", {
			            icon : 1,
			            time : 1000
        ...
        function userDel() {
	        ...
	        if(rid == -1) {
		        ...
		        layer.confirm('确定要删除吗？', { btn : [
			        ...
			        $.post("${basePath}/sys/user/del", { "id" : rid }, function(data) {
				        if(data.code == 0) {
					        layer.msg("操作成功", {
					            icon : 1,
					            time : 1000
