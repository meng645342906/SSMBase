...
<body class="no-skin">
					...
					<div class="col-xs-12">
						<div class="row-fluid" style="margin-bottom: 5px;">
							<div class="span12 control-group">
								<my:button className="btn btn-success" id="btn-visible" textName="启用" />
								<my:button className="btn btn-danger" id="btn-unvisible" textName="禁用" />
								<my:button className="btn btn-primary" id="btn-add" textName="添加" />
								<my:button className="btn btn-info" id="btn-edit" textName="编辑" />
								<my:button className="btn btn-warning" id="btn-del" textName="删除" />
								<my:button className="btn" id="bnt-grant" textName="资源授权" permission="/sys/role" />
							</div>
						</div>
						<!-- PAGE CONTENT BEGINS -->
	...
	<script type="text/javascript">
		$(document).ready(function() {
	        ...
	        $("#grid-table").jqGrid({
	            url : '${basePath}/sys/role/getListData',
	            mtype : "GET",
	            datatype : "json",
	            colModel : [
	                {
	        ...
	        $("#bnt-grant").click(function() {
		        ...
		        if(rid == -1) {
			        ...
			        parent.layer.open({
			            ...
			            fix : false, //不固定
			            maxmin : true,
			            content : '${basePath}/sys/role/getZtree?roleId=' + rid + '&type=1' });
		        }
	        });
	        ...
	        $("#btn-edit").click(function() {//添加页面
		        ...
		        if(rid == -1) {
			        ...
			        parent.layer.open({
			            ...
			            fix : false, //不固定
			            maxmin : true,
			            content : '${basePath}/sys/role/add?id=' + rid });
		        }
	        });
        ...
        function roleDel(){
 	        ...
 	        if(rid == -1) {
 		        ...
 		        layer.confirm('确定要删除吗？', { btn : [
 		            '确定',
 		            '取消' ] //按钮
 		        }, function() {
 			        $.post("${basePath}/sys/role/del", { "id" : rid }, function(data) {
 				        if(data.code == 0) {
 					        layer.msg("操作成功", {
 					            icon : 1,
 					            time : 1000
        ...
        function setVisible(status) {
	        var submitData = {
	            "ids" : getSelectedRows(),
	            "visible" : status };
	        $.post("${basePath}/sys/role/setVisible", submitData, function(data) {
